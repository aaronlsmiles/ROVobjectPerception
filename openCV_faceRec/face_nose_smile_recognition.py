...
def detect(gray, frame): # We create a function that takes as input the image in black and white (gray) and the original image (frame), and that will return the same image with the detector rectangles. 
    ...
    for (x, y, w, h) in faces: # For each detected face:
        ...
        eyes = eye_cascade.detectMultiScale(roi_gray, 1.1, 16) # We apply the detectMultiScale method to locate one or several eyes in the image.
        for (ex, ey, ew, eh) in eyes: # For each detected eye:
            cv2.rectangle(roi_color,(ex, ey),(ex+ew, ey+eh), (0, 255, 0), 2) # We paint a rectangle around the eyes, but inside the referential of the face.
        smiles = smile_cascade.detectMultiScale(roi_gray, 1.7, 23) # We apply the detectMultiScale method to locate one or several eyes in the image.
        for (sx, sy, sw, sh) in smiles: # For each detected eye:
            cv2.rectangle(roi_color,(sx, sy),(sx+sw, sy+sh), (0, 0, 255), 2) # We paint a rectangle around the smile, but inside the referential of the face.      
        noses = nose_cascade.detectMultiScale(roi_gray, 1.3, 23)
        for (nx, ny, nw, nh) in noses: # For each detected eye:
